    ...
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-xl border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <p className="text-gray-700"><strong>Name:</strong> {result.name}</p>
        ...
        <div className="space-y-2">
          <p className="text-gray-700"><strong>Fraud Score:</strong> {result.fraud_score.toFixed(2)}%</p>
          <p className="text-gray-700"><strong>Risk Report:</strong> {result.risk_report || 'None'}</p>
          <p className="text-gray-700"><strong>Appeal Outcome:</strong> {result.appeal_outcome}</p>
          <p className="text-gray-700"><strong>Suggested Action:</strong> {result.suggested_action}</p>
          {result.anomaly_flags.length > 0 && (
            <p className="text-gray-700"><strong>Anomalies:</strong> {result.anomaly_flags.join(', ')}</p>
          )}
        </div>
      </div>
