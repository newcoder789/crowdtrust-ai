...
def create_campaign():
    ...
    ui_image = request.files.get('uiImage')
    ai_check_image = request.files.get('aiCheckImage')
    ...
    campaign_data['uiImage'] = f'http://localhost:5000/uploads/{quote(os.path.basename(ui_path))}'
    campaign_data['aiCheckImage'] = f'http://localhost:5000/uploads/{quote(os.path.basename(ai_path))}'
...
def analyze():
    ...
    if 'uiImage' not in request.files:
        print("ERROR: uiImage is missing!")
        return jsonify({'error': 'Missing uiImage'}), 400
    if 'aiCheckImage' not in request.files:
        print("ERROR: aiCheckImage is missing!")
        return jsonify({'error': 'Missing aiCheckImage'}), 400
    if 'campaign_id' not in request.form:
        print("ERROR: campaign_id is missing!")
    ...
    try:
        campaign = campaigns_collection.find_one({'_id': ObjectId(campaign_id)})
        if not campaign:
            print("ERROR: Campaign not found for ID:", campaign_id)
            return jsonify({'error': 'Campaign not found'}), 404
    except Exception as e:
        print("ERROR: Invalid campaign_id format:", str(e))
        return jsonify({'error': f'Invalid campaign_id: {str(e)}'}), 400
    ...
    if ui_image:
        ui_path = os.path.join('uploads', f'{datetime.now().timestamp()}-{quote(ui_image.filename)}')
        ui_image.save(ui_path)
        logging.info(f"Saved UI image to {ui_path}")
    if ai_check_image:
        ai_path = os.path.join('uploads', f'{datetime.now().timestamp()}-{quote(ai_check_image.filename)}')
        ai_check_image.save(ai_path)
        logging.info(f"Saved AI check image to {ai_path}")
    # Build campaign data
    campaign_data = {
        'name': campaign['name'],
        'description': campaign.get('description', ''),
    ...
    # Store report and update campaign
    report_id = fraud_reports_collection.insert_one(fraud_report).inserted_id
    campaigns_collection.update_one(
        {'_id': ObjectId(campaign_id)},
        {'$push': {'fraud_report_ids': str(report_id)}}
...
@cross_origin(origins="http://localhost:5173")  # Install flask_cors if not already
